...
API endpoints contains the controllers which handles the routing and requests.

DataAccess contains the DataContext for the database.
For the sake of simplicity, we have decided to utilize a local database.

DataAccess.Maintenance contains the migrations for the project.
...
The Model module contains the different models we use.
There is one for receipts and one for image, there is also a data transfer object for both of them.

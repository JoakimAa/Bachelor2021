...
In image~\ref{OCR matching} you can see the algorithm finding matches between a Trøndertaxi receipt from two
different years.
...
We combine the results to get a 2x2 max pool output, down from a 4x4 format.\ref{Max pooling}

\begin{figure}[h]
    \center{\includegraphics[width=0.8 \textwidth]{Images/maxpooling2D}}
    \caption{Overview for max pooling (Image from~\cite{CNN})}

...
The .NET API exposes several endpoints that allows for saving an image and a receipt to a database.
The .NET API is also connected to the CNN and OCR module via the ML API\@.
...
Finally, the outputs of the CNN and OCR are combined in Spacy and the data is provided back to the user.
...
The user sends in an image via a website and the API handles the request and sends the data to the ML API\@.

When the data has been processed through the CNN and OCR/Spacy, the API sends the data back to the
user.
...
the image the user sends in, and then send the data back to the .NET API.
